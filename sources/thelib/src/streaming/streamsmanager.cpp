...
bool StreamsManager::RegisterStream(BaseStream *pStream) {
	FINEST("**StreamsManager::RegisterStream, type:%lu, name:%s, unique ID:%u, app name:%s", 
		pStream->GetType(), pStream->GetName().c_str(), pStream->GetUniqueId(), _pApplication->GetName().c_str());	//add by zhanglei for test
